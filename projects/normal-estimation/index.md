...
title: "Didactic Guide: Normal Estimation & Orientation Propagation"
subtitle: "An interactive guide to graph-based geometry processing"
custom_scripts:
...
steps:
  - title: "1. Local Neighborhoods"
    description: >
      To estimate local geometry at a point $\htmlClass{math-term-pi-point}{\mathbf{p}_i}$ on a raw point cloud, we must first establish its local context. We construct a neighborhood $\htmlClass{math-term-ni}{\mathcal{N}(\mathbf{p}_i)}$ of nearby points, typically using the $k$-nearest neighbors ($k$-NN) or a spatial radius search ($\epsilon$-neighborhood):

      $$ \htmlClass{math-term-ni}{\mathcal{N}(\mathbf{p}_i)} = \{ \htmlClass{math-term-pj-point}{\mathbf{p}_j} \in \mathcal{P} \mid \|\htmlClass{math-term-pj-point}{\mathbf{p}_j} - \htmlClass{math-term-pi-point}{\mathbf{p}_i}\| \le \htmlClass{math-term-epsilon}{\epsilon} \} $$
    ...
    takeaway: '💡 **Key Insight:** Choosing the neighborhood size ($\htmlClass{math-term-epsilon}{\epsilon}$) is a fundamental scale problem: if too small, noise dominates; if too large, fine geometric details and sharp corners are smoothed out.'

  - title: "2. Principal Component Analysis (PCA)"
    description: >
      For each point $\mathbf{p}_i$, we analyze the spatial layout of its neighborhood $\mathcal{N}(\mathbf{p}_i)$ by computing the symmetric covariance matrix $\htmlClass{math-term-ci}{\mathbf{C}_i}$ relative to the neighborhood's centroid $\htmlClass{math-term-centroid}{\bar{\mathbf{p}}_i}$:

      $$ \htmlClass{math-term-ci}{\mathbf{C}_i} = \frac{1}{|\mathcal{N}(\mathbf{p}_i)|} \sum_{j \in \mathcal{N}(\mathbf{p}_i)} (\mathbf{p}_j - \htmlClass{math-term-centroid}{\bar{\mathbf{p}}_i})(\mathbf{p}_j - \htmlClass{math-term-centroid}{\bar{\mathbf{p}}_i})^T $$

      The eigenvectors of $\mathbf{C}_i$ define the principal directions of variance. The eigenvector associated with the smallest eigenvalue $\htmlClass{math-term-lambda}{\lambda_0}$ points in the direction of minimum variance: our estimated normal vector $\htmlClass{math-term-ni-vec}{\mathbf{n}_i}$.
  ...
  - title: "3. Minimum Spanning Tree (MST) Orientation Propagation"
    description: >
      ...
      $$ \htmlClass{math-term-wij}{w_{ij}} = 1 - |\htmlClass{math-term-dot}{\mathbf{n}_i \cdot \mathbf{n}_j}| $$
    ...
    takeaway: '💡 **Key Insight:** Minimizing $1 - |\htmlClass{math-term-dot}{\mathbf{n}_i \cdot \mathbf{n}_j}|$ ensures we propagate orientations along flat regions first, avoiding propagation across sharp folds or noisy seams where orientations can break.'
---
